...
- Define class TicTacToe
 - checkWinner(board) -> bool
  ...
  - OPTIMIZATION: We do not need to run check for every possible win on every tile. Start at top left and if index falls in category below, we perform only tthe necessary checks
   ...
   - Box -> DO NOT check right most column and bottom most row
...
O(n) where n is number of pieces on the board since we must visit every piece on the board
...
- generate all possible win combinations (tuple)
...
- walk each pattern for x and then for o
 - iterate through each possible win combination and check if they all belong to the same player... if they do winner if not next patter

Same O(n) where n is numbber of win conditions as input size increases
...
- Option 1
 ...
 - Soupy logic - a lot of conditionals -> harder to edit in future
 - Harder to unit test?
 - More intuitive

- Option 2
 - Harder to read
 - reuses maching pattern


# Edge Cases/Errors:
- Invalid board? -> validate in separate function
